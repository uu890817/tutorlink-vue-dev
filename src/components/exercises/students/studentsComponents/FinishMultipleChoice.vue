<template>
    <n-card :title="'(' + props.index + ') ' + topicData.content + '(多選)'">
        <!-- {{ topicData }} -->
        <n-space vertical>
            <p>你的答案:</p>
            <n-checkbox-group v-model:value="yourAnswer">
                <n-space item-style="display: flex;" v-for="option in choiceOptions">
            ...
            <n-collapse>
                <n-collapse-item title="點我看正確答案" name="1">
                    <n-checkbox-group v-model:value="correctAnswer">
                        <n-space item-style="display: flex;" v-for="option in choiceOptions">
                            <n-checkbox :value="option.value" :label="option.label" />
                        </n-space>
                    </n-checkbox-group>
        ...
        <template #header-extra>
            <n-icon color="#119911" size="30" v-if="JSON.stringify(yourAnswer) === JSON.stringify(correctAnswer)">
                <MdCheckmarkCircle />
            </n-icon>
            <n-icon color="#dd1111" size="30" v-if="JSON.stringify(yourAnswer) !== JSON.stringify(correctAnswer)">
                <MdCloseCircle />
            </n-icon>
        </template>
    </n-card>
...
const yourAnswer = computed(() => {
    return (props.data.answer).split('<AND>')
...
const correctAnswer = computed(() => {
    ...
    for (let i = 0; i < topicData.value.options.length; i++) {
        if (topicData.value.options[i].answer === 'true') {
            answer.push(topicData.value.options[i].content)
        }
    }
    return answer
})
